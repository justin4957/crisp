...
- Mutable assignment: basic, after let binding, with function call value (issue #187)
- Type cast expressions: simple `as` cast, with type constructor, in function body, ECast node verification, with refinement type, precedence below arithmetic (issue #240)
...
- Authority as field name (issue #221)
- Authority as parameter name (issue #221)
...
**Expression Formatting (~20 tests)**
...
- Type cast expressions: `as` cast formatting and idempotency (issue #240)
...
- Mutable assignment: formatting and idempotency (issue #187)
...
- Provides block: with effect (issue #160)
- Provides block: effect idempotent formatting (issue #160)
- Provides block: with trait (issue #164)
- Provides block: trait idempotent formatting (issue #164)
...
**Trait Definition Desugaring Tests (issue #264)**
- Simple trait with one method
- Trait with type parameter: `Ord A`
- Trait with multiple methods: `Num A` (add, sub, mul)
- Trait with Self type in method signature: `clone(self) -> Self`
